...
public class DriveSyncer {
	...
	public static Collection<String> getScope() {
		String scope = parent.equals("appdata") ? "https://www.googleapis.com/auth/drive.appdata"
				: DriveScopes.DRIVE;
        Collection<String> scopeColection = new LinkedList<String>();
        scopeColection.add(scope);
        return scopeColection;
	}
			...
			try {
				...
				if (mainCredentials == null) {
					credential = GoogleAccountCredential.usingOAuth2(mContext,
                                DriveSyncer.getScope()); // DriveScopes.DRIVE_FILE);
	...
	private void mergeEpisodes(Map<String, File> files) {
        ...
        // http://stackoverflow.com/questions/12177889/java-util-concurrentmodificationexception-while-accessing-hashmap
		while (it.hasNext()) {

            Map.Entry<String, File> item = it.next();
	...
	private List<WithIcon> getPlaylist() {
		...
		try {
			itemCursor = mProvider.query(itemUri, ITEM_PROJECTION,
					...
					PlaylistFragment.getOrder("DESC", 20));
		} catch (RemoteException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
